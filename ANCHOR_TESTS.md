...
- `crates/agent/src/decisions.rs::tests::append_chained_locked_refuses_non_iso_date_segment` - end-to-end pin: passing a non-ISO-date `today` to `append_chained_locked` errors BEFORE any `open()`/`lock_exclusive()` syscall.
